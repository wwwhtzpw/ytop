...
-- Purpose: SQL tuning report with LITERAL SQL, sql.sql plan and EXPLAIN PLAN FOR
-- Created: 20260802  by  huangtingzhong
-- Notes:
--   Front: (1) LITERAL SQL (2) PLAN from v$sql_plan like sql.sql (Id|Pid|Ord|...)
--          (3) EXPLAIN PLAN FOR paste (re-run as -f for PLAN_DESCRIPTION / Param).
--   Back: same report sections as sql.sql / plan_pred_by_sqlid.
--   1) LITERAL SQL: binds replaced with captured values (print only).
--   2) Plan: pure SELECT from v$sql_plan (sql.sql layout).
--   3) EXPLAIN PLAN FOR: original sql_fulltext printed for manual re-run (predicates).
--   4) CURRENT_SCHEMA from parsing_schema_name per child.
--   5) sql_fulltext as CLOB (UTF8 chunk=1000).
--   6) Optional child_number filters LITERAL/EXPLAIN/BIND/plan; objects use sql_id.
  ...
  SELECT p.plan_hash_value AS phv,
   ...
   WHERE p.sql_id = '&&sqlid'
     AND (TRIM('&&child') IS NULL
          OR TO_CHAR(p.child_number) = TRIM('&&child'))
      ...
      FROM (
            SELECT child_number, parsing_schema_name, sql_fulltext,
             ...
             WHERE s.sql_id = c_sqlid
               AND (c_child_in IS NULL OR TO_CHAR(s.child_number) = c_child_in)
