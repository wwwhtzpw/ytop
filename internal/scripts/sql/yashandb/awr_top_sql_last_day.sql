...
-- Purpose: YashanDB Top-N AWR SQL last N days with plan and objects
...
-- Updated: 20260809 by huangtingzhong (ACCEPT days/username; schema filter)
-- Updated: 20260809 by huangtingzhong (ACCEPT exclude_user; schema exclude)
-- Updated: 20260809 by huangtingzhong (username/exclude_user CSV multi-schema)
--
-- Params:
--   days         : last N calendar days (Enter=1, aka last day); must be > 0
--   username     : include schema(s), comma-separated (Enter=no filter; exact, CI)
--   exclude_user : exclude schema(s), comma-separated (Enter=no exclude; exact, CI)
...
PROMPT
PROMPT +------------------------------------------------------------------------+
PROMPT | AWR Top-N SQL (last N days)                                            |
...
DECLARE
    ...
    v_user      VARCHAR2(4000);         -- normalized CSV include (NULL=all)
    v_excl      VARCHAR2(4000);         -- normalized CSV exclude (NULL=none)
    v_days      NUMBER;                 -- last N days (default 1)
    v_top_n     NUMBER        := 10;    -- top N SQL_ID by CPU
    ...
    -- schema filter: v_user/v_excl are normalized CSV (UPPER, no spaces); NULL = no filter
    CURSOR c_top_sql IS
        WITH snap_range AS (
...
BEGIN
    ...
    -- parse days: Enter -> 1; reject non-numeric / <= 0
    -- Yashan TO_NUMBER on bad text may raise YAS-00008 (not VALUE_ERROR)
    ...
    -- normalize CSV: UPPER, trim tokens, drop empties/dupes
    DECLARE
    ...
    put_line('collect_top_sql: start'
        || ' days=' || TO_CHAR(v_days)
        || ' user=' || NVL(v_user, 'ALL')
    ...
    FOR rec IN c_top_sql LOOP
        ...
        put_line(RPAD('PHV',10) || ' ' || RPAD('EXEC',8) || ' ' || RPAD('CPU_PER',10) || ' ' || RPAD('ELA_PER',10) || ' ' || RPAD('DISK_PER',9) || ' ' || RPAD('AVG_GETS',9) || ' ' || RPAD('ROWS_PER',9) || ' ' || RPAD('ROW_FETCH',10) || ' ' || RPAD('APP_PER',10) || ' ' || RPAD('CON_WPER',10) || ' ' || RPAD('CLU_PER',10) || ' ' || RPAD('UIO_PER',10) || ' ' || RPAD('PLSQL_PER',10) || ' ' || RPAD('OUTLINE',18));
        ...
        put_line( '+------------------------------------------------------------------------+');
        put_line( '');
        put_line(RPAD('EXEC',8) || ' ' || RPAD('PHV',10) || ' ' || RPAD('C',3) || ' ' || RPAD('USERNAME',15) || ' ' || RPAD('CPU_PER',10) || ' ' || RPAD('ELA_PER',10) || ' ' || RPAD('DISK_PER',9) || ' ' || RPAD('AVG_GETS',9) || ' ' || RPAD('ROWS_PER',9) || ' ' || RPAD('ROW_FETCH',10) || ' ' || RPAD('APP_PER',10) || ' ' || RPAD('CON_PER',10) || ' ' || RPAD('CLU_PER',10) || ' ' || RPAD('UIO_PER',10) || ' ' || RPAD('PLSQL_PER',10) || ' ' || RPAD('F_L_TIME',12));
        ...
        put_line( '+------------------------------------------------------------------------+');
        put_line( '');
        put_line(RPAD('END_TIME',8) || ' ' || RPAD('I',2) || ' ' || RPAD('USERNAME',15) || ' ' || RPAD('PHV',10) || ' ' || RPAD('EXEC',8) || ' ' || RPAD('CPU_PER',10) || ' ' || RPAD('ELA_PER',10) || ' ' || RPAD('DISK_PER',9) || ' ' || RPAD('AVG_GETS',9) || ' ' || RPAD('ROWS_PER',9) || ' ' || RPAD('ROW_FETCH',10) || ' ' || RPAD('WRITE_PER',9) || ' ' || RPAD('IOWAIT_PER',10) || ' ' || RPAD('SORT_PER',10) || ' ' || RPAD('APP_PER',10) || ' ' || RPAD('CON_PER',10) || ' ' || RPAD('CLU_PER',10) || ' ' || RPAD('PLSQL_PER',10));
