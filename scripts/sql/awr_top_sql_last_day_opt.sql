...
-- Purpose: Collect top-N SQL by CPU from AWR for the last day (optimized: objects
...
DECLARE
    ...
    -- 数值格式：<1000 保留 2 位小数，<10000 为 K，否则为 W（用于 EXEC/DISK_GETS/ROWS 等）
...
BEGIN
    ...
    IF sql_id_list.COUNT = 0 THEN
        ...
        RETURN;
    END IF;
    -- 2) 构建 SQL_ID IN 列表，供统一对象收集与性能数据查询使用
    in_list_sql_id := '';
    ...
    -- 4) 按 SQL_ID 循环：仅输出 SQL 性能相关段（带 SQL_ID 区分），对象段在后面统一输出一次
    FOR sql_idx IN 1..sql_id_list.COUNT LOOP
        ...
        -- LITERAL SQL：从 V$SQL 取 SQL_FULLTEXT，若有绑定则从 V$SQL_BIND_CAPTURE 替换后输出（简化：仅输出 schema + 原文一段）
        DECLARE
    ...
    -- 4b) SQL 性能数据统一查询一次（WHERE sql_id IN），带 SQL_ID 列区分
    put_line( '');
    put_line( '+------------------------------------------------------------------------+');
    put_line( '| infromation  from v$sqlstats (all TOP SQLs, one query)                 |');
    put_line( '+------------------------------------------------------------------------+');
    put_line( '');
    put_line(RPAD('SQL_ID',14) || RPAD('PLAN_HASH_VALUE',15) || RPAD('EXEC',10) || RPAD('CPU_PRE_EXEC',12) || RPAD('ELA_PRE_EXEC',12) || RPAD('DISK_PRE_EXEC',13) || RPAD('GET_PRE_EXEC',12) || RPAD('ROWS_PRE_EXEC',13) || RPAD('ROWS_PRE_FETCHES',15) || RPAD('APP_WAIT_PRE',12) || RPAD('CON_WAIT_PER',12) || RPAD('CLU_WAIT_PER',12) || RPAD('USER_IO_WAIT_PER',15) || RPAD('PLSQL_WAIT_PER',14) || RPAD('OUTLINE',20));

    DECLARE
        sqlarea_coll t_sqlarea_tab;
        v_sq VARCHAR2(32767);
    BEGIN
        v_sq := 'SELECT sql_id AS r_sql_id, PLAN_HASH_VALUE||'''' AS plan_hash_value, EXECUTIONS AS executions, '
        ...
        FOR i IN 1..NVL(sqlarea_coll.COUNT,0) LOOP
            v_line := RPAD(SUBSTR(NVL(sqlarea_coll(i).r_sql_id,' '),1,14),14) || RPAD(SUBSTR(NVL(sqlarea_coll(i).plan_hash_value,' '),1,15),15)
                || RPAD(SUBSTR(NVL(fmt_num_kw(sqlarea_coll(i).executions),' '),1,10),10) || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).cpu_per_exec),' '),1,12),12)
                || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).ela_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_num_kw(sqlarea_coll(i).disk_per_exec),' '),1,13),13)
                || RPAD(SUBSTR(NVL(fmt_num_kw(sqlarea_coll(i).get_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_num_kw(sqlarea_coll(i).rows_per_exec),' '),1,13),13)
                || RPAD(SUBSTR(NVL(fmt_num_kw(sqlarea_coll(i).rows_per_fetches),' '),1,15),15) || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).app_wait_per),' '),1,12),12)
                || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).con_wait_per),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).clu_wait_per),' '),1,12),12)
                || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).user_io_wait_per),' '),1,15),15) || RPAD(SUBSTR(NVL(fmt_time_us(sqlarea_coll(i).plsql_wait_per),' '),1,14),14)
                || RPAD(SUBSTR(NVL(sqlarea_coll(i).outline,' '),1,20),20);
            put_line(v_line);
        END LOOP;
    END;

    -- 4c) v$sql 统一一次查询（WHERE sql_id IN），带 SQL_ID 列
    put_line( '');
    put_line( '+------------------------------------------------------------------------+');
    put_line( '| information from v$sql (all TOP SQLs, one query)                      |');
    put_line( '+------------------------------------------------------------------------+');
    put_line( '');
    put_line(RPAD('SQL_ID',14) || RPAD('EXEC',10) || RPAD('PLAN_HASH_VALUE',15) || RPAD('C',3) || RPAD('USERNAME',15) || RPAD('CPU_PRE_EXEC',12) || RPAD('ELA_PRE_EXEC',12) || RPAD('DISK_PRE_EXEC',13) || RPAD('GET_PRE_EXEC',12) || RPAD('ROWS_PRE_EXEC',13) || RPAD('ROWS_PRE_FETCHES',15) || RPAD('APP_PRE_EXEC',12) || RPAD('CON_PRE_EXEC',12) || RPAD('CLU_WAIT_PER',12) || RPAD('USER_IO_WAIT_PER',15) || RPAD('PLSQL_WAIT_PER',14) || RPAD('F_L_TIME',15));

    DECLARE
        vsql_coll t_vsql_tab;
        v_sq2 VARCHAR2(32767);
    BEGIN
        v_sq2 := 'SELECT sql_id AS r_sql_id, PLAN_HASH_VALUE||'''' AS plan_hash_value, child_number||'''' AS c, PARSING_SCHEMA_NAME AS username, '
            ...
            || 'FROM v$sql WHERE sql_id IN (' || in_list_sql_id || ') ORDER BY sql_id, plan_hash_value';
        EXECUTE IMMEDIATE v_sq2 BULK COLLECT INTO vsql_coll;
        FOR i IN 1..NVL(vsql_coll.COUNT,0) LOOP
            v_line := RPAD(SUBSTR(NVL(vsql_coll(i).r_sql_id,' '),1,14),14) || RPAD(SUBSTR(NVL(fmt_num_kw(vsql_coll(i).executions),' '),1,10),10) || RPAD(SUBSTR(NVL(vsql_coll(i).plan_hash_value,' '),1,15),15) || RPAD(SUBSTR(NVL(vsql_coll(i).c,' '),1,3),3) || RPAD(SUBSTR(NVL(vsql_coll(i).username,' '),1,15),15)
                || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).cpu_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).ela_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_num_kw(vsql_coll(i).disk_per_exec),' '),1,13),13) || RPAD(SUBSTR(NVL(fmt_num_kw(vsql_coll(i).get_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_num_kw(vsql_coll(i).rows_per_exec),' '),1,13),13) || RPAD(SUBSTR(NVL(fmt_num_kw(vsql_coll(i).rows_per_fetches),' '),1,15),15)
                || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).app_pre_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).con_pre_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).clu_wait_per),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).user_io_wait_per),' '),1,15),15) || RPAD(SUBSTR(NVL(fmt_time_us(vsql_coll(i).plsql_wait_per),' '),1,14),14) || RPAD(SUBSTR(NVL(vsql_coll(i).f_l_time,' '),1,15),15);
            put_line(v_line);
        END LOOP;
    END;
        ...
        put_line( '+------------------------------------------------------------------------+');
        put_line( '');
        put_line(RPAD('SQL_ID',14) || RPAD('END_TIME',8) || RPAD('I',2) || RPAD('USERNAME',15) || RPAD('PLAN_HASH_VALUE',15) || RPAD('EXEC',10) || RPAD('CPU_PRE_EXEC',12) || RPAD('ELA_PRE_EXEC',12) || RPAD('DISK_PRE_EXEC',13) || RPAD('GET_PRE_EXEC',12) || RPAD('ROWS_PRE_EXEC',13) || RPAD('ROWS_PRE_FETCHES',15) || RPAD('WRITE_PRE_EXEC',14) || RPAD('IOWAIT_PRE_EXEC',15) || RPAD('SORTS_PRE_EXEC',15) || RPAD('APP_PRE_EXEC',12) || RPAD('CON_PRE_EXEC',12) || RPAD('CLU_WAIT_PER',12) || RPAD('PLSQL_WAIT_PER',14));

    DECLARE
        awr_coll t_awr_tab;
        v_sq_awr VARCHAR2(32767);
    BEGIN
        v_sq_awr := 'SELECT a.sql_id AS r_sql_id, TO_CHAR(b.END_INTERVAL_TIME, ''dd hh24'') AS end_time, TRIM(TO_CHAR(a.instance_number)) AS i, a.parsing_schema_name AS username, a.plan_hash_value||'''' AS plan_hash_value, '
        ...
        FOR i IN 1..NVL(awr_coll.COUNT,0) LOOP
            v_line := RPAD(SUBSTR(NVL(awr_coll(i).r_sql_id,' '),1,14),14) || RPAD(SUBSTR(NVL(awr_coll(i).end_time,' '),1,8),8) || RPAD(SUBSTR(NVL(awr_coll(i).i,' '),1,2),2) || RPAD(SUBSTR(NVL(awr_coll(i).username,' '),1,15),15) || RPAD(SUBSTR(NVL(awr_coll(i).plan_hash_value,' '),1,15),15)
                || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).executions),' '),1,10),10) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).cpu_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).ela_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).disk_per_exec),' '),1,13),13) || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).get_per_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).rows_per_exec),' '),1,13),13) || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).rows_per_fetches),' '),1,15),15) || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).write_per_exec),' '),1,14),14) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).iowait_per_exec),' '),1,15),15) || RPAD(SUBSTR(NVL(fmt_num_kw(awr_coll(i).sorts_per_exec),' '),1,15),15) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).app_pre_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).con_pre_exec),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).clu_wait_per),' '),1,12),12) || RPAD(SUBSTR(NVL(fmt_time_us(awr_coll(i).plsql_wait_per),' '),1,14),14);
            put_line(v_line);
        END LOOP;
    END;

    -- 5) 对象段只输出一次（不展示 SQL_ID）
    put_line( '');
    put_line( '****************************************************************************************');
...
-- ============================================================================
-- 验证方法（collect_top.sql 优化版）：
-- 1) 先单独执行 awr_sql_by_cpu_by_day.sql：设置 days_back=1, days_show=1，得到当日 TOP 10 的 SQL_ID 列表。
-- 2) 执行本脚本：所有 TOP SQL_ID 统一传入，对象（表/索引/分区等）只收集一次，无重复。
-- 3) SQL 性能数据段（v$sqlstats、v$sql、AWR）每行带 SQL_ID 列便于区分；对象段（OBJECT SIZE/TABLES/INDEX 等）只输出一次且不展示 SQL_ID。
-- 4) 与 sql.sql 对比时：性能段列名与顺序一致且含 SQL_ID；对象段与 collect_top_sql_last_day 单 SQL 结果一致（对象并集）。
-- ============================================================================
